...
const AUTO_TRANSLATIONS: Record<string, Record<string, string>> = {
  ...
  es: {
    ...
    'Agent results will be displayed here': 'Los resultados del agente se mostrarán aquí',
  ...
  fr: {
    ...
    'Agent results will be displayed here': "Les résultats de l'agent s'afficheront ici",
  ...
  de: {
    ...
    'Agent results will be displayed here': 'Agent-Ergebnisse werden hier angezeigt',
  ...
  ja: {
    ...
    'Confidence': '信頼度',
    'Tokens used': '使用トークン',
    'Agent results will be displayed here': 'エージェントの結果がここに表示されます',
    'Search Results': '検索結果',
 ...
 */
function generateLocaleTranslations(locale: string, baseTranslations: Record<string, any>): void {
  ...
  const translatedStructure = translateObject(baseTranslations, localeTranslations, locale);

  // Write the file
  fs.writeFileSync(agentsFilePath, JSON.stringify(translatedStructure, null, 2), 'utf-8');
...
async function main(): Promise<void> {
  ...
  const baseFilePath = path.join(__dirname, '..', 'public', 'locales', 'en', 'agents.json');
  
  if (!fs.existsSync(baseFilePath)) {
    console.error('❌ English agents.json template not found!');
    console.error('Please ensure the base English file exists at:', baseFilePath);
    process.exit(1);
  }
  ...
  if (generated > 0) {
    ...
    console.log('   Professional translators should review and refine these translations.');
    console.log('   Look for entries marked with [LOCALE] prefix for missing translations.');
  }
