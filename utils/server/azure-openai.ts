...
export default class AzureOpenAIClient {
        ...
        switch (type) {
            case AzureOpenAIClientCredentialsType.VISION:
                if(this.visionHostName && this.visionApiKey && this.visionDeployment)
                    return { host: this.visionHostName, apiKey: this.visionApiKey, deployment: this.visionDeployment }
                else
                    throw new Error('VISION credentials are incomplete')
            case AzureOpenAIClientCredentialsType.DALLE:
                if(this.dalleHostName && this.dalleApiKey && this.dalleDeployment)
                    return { host: this.dalleHostName, apiKey: this.dalleApiKey, deployment: this.dalleDeployment }
                else
                    throw new Error('DALLE credentials are incomplete')
            case AzureOpenAIClientCredentialsType.WHISPER:
                if(this.whisperHostName && this.whisperApiKey && this.whisperDeployment)
                    return { host: this.whisperHostName, apiKey: this.whisperApiKey, deployment: this.whisperDeployment }
                else
                    throw new Error('WHISPER credentials are incomplete')
            case AzureOpenAIClientCredentialsType.DEFAULT:
            default:
                return { host: this.hostName, apiKey: this.apiKey, deployment: this.deployment }
        }
    }
    ...
    async submit(messages: Message[], credentials: AzureOpenAIClientCredentials): Promise<any> {
    ...
    async getChatCompletion(messages: Message[]): Promise<any> {
        const credentials = this.getCredentials(AzureOpenAIClientCredentialsType.DEFAULT);
    ...
    async getVisionCompletion(messages: Message[]): Promise<any> {
        ...
        try {
            credentials = this.getCredentials(AzureOpenAIClientCredentialsType.VISION);
        } catch (error) {
            credentials = this.getCredentials(AzureOpenAIClientCredentialsType.DEFAULT)
        }
        return this.submit(messages, credentials);
    }

    async getDalleCompletion(messages: Message[]): Promise<any> {
        const credentials = this.getCredentials(AzureOpenAIClientCredentialsType.DALLE);
        return this.submit(messages, credentials);
    }
