...
export class SimpleKnowledgeLoader {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const privacyItems: SimpleKnowledgeItem[] = privacyData.items.map(
        (item) => ({
          id: item.id,
      ...
      console.log(
        `[INFO] Loaded ${this.knowledgeItems.length} knowledge items (${faqItems.length} FAQ, ${privacyItems.length} privacy)`,
      );
      ...
      throw new Error(
        `Knowledge loader initialization failed: ${
  ...
  async search(
    ...
    console.log(
      `[INFO] Searching knowledge base for: "${query}" (type: ${
        type || 'all'
      })`,
    );
    ...
    console.log(
      `[INFO] Found ${limitedResults.length} relevant knowledge items (${results.length} total above threshold)`,
    );
  ...
  getStatistics() {
    return {
      ...
      faqItems: this.knowledgeItems.filter((item) => item.type === 'faq')
        .length,
      privacyItems: this.knowledgeItems.filter(
        (item) => item.type === 'privacy_policy',
      ).length,
      initialized: this.initialized,
      categories: [
        ...new Set(this.knowledgeItems.map((item) => item.category)),
      ],
    };
  }
  ...
  private scoreItem(
    ...
    for (const keyword of item.keywords) {
      ...
      if (normalizedQuery.includes(normalizedKeyword)) {
        ...
        const matchingWords = keywordWords.filter((word) =>
          queryWords.some(
            (queryWord) => queryWord.includes(word) || word.includes(queryWord),
          ),
        );

        if (matchingWords.length > 0) {
          const partialScore =
            (matchingWords.length / keywordWords.length) *
            this.config.partialMatchWeight;
          score += partialScore;
          matchedKeywords.push(keyword);
          explanations.push(
            `partial match: "${keyword}" (${matchingWords.join(', ')})`,
          );
        }
      }
    }
    ...
    if (item.question) {
      ...
      if (questionMatches.length > 0) {
        ...
        explanations.push(
          `question content match: ${questionMatches.length} words`,
        );
      }
    }
 ...
 */
export function getKnowledgeLoader(
