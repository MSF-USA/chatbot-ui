      ...
      return NextResponse.json(
        {
          ...
          details: 'Server configuration error. Please contact your administrator.'
        },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      if (agentError.statusCode === 403 || agentError.code === 'Forbidden') {
        return NextResponse.json(
          {
            error: 'Access denied',
            details: 'The server does not have permission to access this agent. Please contact your administrator.'
          },
          { status: 403 }
        );
      }
      ...
      return NextResponse.json(
        {
          ...
          details: agentError.message || 'Unable to connect to the agent. Please verify the Agent ID and try again.'
        },
        { status: 500 }
      );
    }
