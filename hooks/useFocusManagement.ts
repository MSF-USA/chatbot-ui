 ...
 * Focus Management Hook
 */
export function useFocusManagement(config: Partial<FocusTrapConfig> = {}): UseFocusManagementReturn {
  ...
  const getFocusableElements = useCallback((container?: HTMLElement): HTMLElement[] => {
    ...
    const elements = Array.from(root.querySelectorAll(FOCUSABLE_SELECTORS)) as HTMLElement[];
   ...
   */
  const focusElement = useCallback((element: HTMLElement | string): boolean => {
    try {
   ...
   */
  const focusNext = useCallback((current?: HTMLElement): boolean => {
   ...
   */
  const focusPrevious = useCallback((current?: HTMLElement): boolean => {
    const currentElement = current || (document.activeElement as HTMLElement);
    const elements = getFocusableElements();
    const currentIndex = elements.indexOf(currentElement);
  ...
  }, [isTrapped, finalConfig.escapeDeactivates, getFocusableElements, focusElement]);

  /**
   * Handle click outside for focus trap
   */
  const handleTrapClick = useCallback((e: MouseEvent) => {
    if (!isTrapped || !trapRef.current || !finalConfig.clickOutsideDeactivates) return;
