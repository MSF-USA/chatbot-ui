...
export const ValidatedInput = forwardRef<HTMLInputElement, ValidatedInputProps>(({
  ...
  // State
  const [value, setValue] = useState(inputProps.defaultValue?.toString() || '');
   ...
   */
  const validateValue = useCallback(async (inputValue: string): Promise<ValidationResult> => {
    const errors: string[] = [];
      ...
      switch (rule.type) {
        ...
        case 'minLength':
          if (inputValue.length < (rule.value || 0)) {
            errors.push(rule.message || t('Must be at least {{min}} characters', { min: rule.value }));
          }
          break;
        ...
        case 'maxLength':
          if (inputValue.length > (rule.value || Infinity)) {
            if (inputValue.length > (rule.value || 0) * 1.1) {
              errors.push(rule.message || t('Must be no more than {{max}} characters', { max: rule.value }));
            } else {
              warnings.push(t('Approaching character limit ({{current}}/{{max}})', { 
        ...
        case 'email':
          if (inputValue && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(inputValue)) {
            errors.push(rule.message || t('Please enter a valid email address'));
          }
          break;
   ...
   */
  const debouncedValidate = useCallback(async (inputValue: string) => {
  ...
  }, [onChange, validation, validateOnChange, hasInteracted, debouncedValidate]);
    ...
    <div className={`space-y-1 ${className}`}>
      ...
      <div className="relative">
        <input
          ...
          onBlur={handleBlur}
          onFocus={handleFocus}
          className={`${getInputStyling()} ${type === 'password' ? 'pr-20' : 'pr-10'}`}
      ...
      {helperText && !validation.errors.length && !validation.warnings.length && (
        <p id={finalHelperId} className="text-sm text-gray-600 dark:text-gray-400">
          {helperText}
        </p>
      )}
          ...
          {validation.errors.length > 0 && (
            <div id={finalErrorId} role="alert" aria-live="polite">
              {validation.errors.map((error, index) => (
                <p key={index} className="text-sm text-red-600 dark:text-red-400 flex items-start gap-1">
          ...
          {validation.warnings.length > 0 && validation.errors.length === 0 && (
            <div role="status" aria-live="polite">
              {validation.warnings.map((warning, index) => (
                <p key={index} className="text-sm text-yellow-600 dark:text-yellow-400 flex items-start gap-1">
