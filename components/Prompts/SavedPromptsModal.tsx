...
interface SavedPromptsModalProps {
  ...
  toneFolders: FolderInterface[];
  collapsedFolders: Set<string>;
  collapsedToneFolders: Set<string>;
  onToggleFolder: (folderId: string) => void;
  onToggleToneFolder: (folderId: string) => void;
  onCreateFolder: () => void;
  onCreateToneFolder: () => void;
  onRenameFolder: (folderId: string, currentName: string) => void;
  onRenameToneFolder: (folderId: string, currentName: string) => void;
  onDeleteFolder: (folderId: string, e: React.MouseEvent) => void;
  onDeleteToneFolder: (folderId: string, e: React.MouseEvent) => void;
  onEditPrompt: (prompt: Prompt) => void;
  onEditTone: (tone: Tone) => void;
  onDeletePrompt: (promptId: string, e: React.MouseEvent) => void;
  onDeleteTone: (toneId: string, e: React.MouseEvent) => void;
  onMovePromptToFolder: (promptId: string, folderId: string | null) => void;
  onMoveToneToFolder: (toneId: string, folderId: string | null) => void;
  onCreatePrompt: () => void;
  onCreateTone: () => void;
  onImportPrompts: (newPrompts: Prompt[]) => void;
  onImportTones: (newTones: Tone[]) => void;
  editingFolderId: string | null;
  editingToneFolderId: string | null;
  editingFolderName: string;
  editingToneFolderName: string;
  onEditingFolderNameChange: (name: string) => void;
  onEditingToneFolderNameChange: (name: string) => void;
  onSaveFolderName: () => void;
  onSaveToneFolderName: () => void;
  onCancelEditFolder: () => void;
  onCancelEditToneFolder: () => void;
}
...
export function SavedPromptsModal({
  ...
  editingToneFolderId,
  editingFolderName,
  editingToneFolderName,
  onEditingFolderNameChange,
  onEditingToneFolderNameChange,
  onSaveFolderName,
  onSaveToneFolderName,
  onCancelEditFolder,
  onCancelEditToneFolder,
}: SavedPromptsModalProps) {
  const t = useTranslations();
  const [activeTab, setActiveTab] = useState<'prompts' | 'tones'>('prompts');
  const [promptSearchQuery, setPromptSearchQuery] = useState('');
  const [toneSearchQuery, setToneSearchQuery] = useState('');
  const [selectedPromptId, setSelectedPromptId] = useState<string | null>(null);
  const [selectedToneId, setSelectedToneId] = useState<string | null>(null);
  const [dragOverFolderId, setDragOverFolderId] = useState<string | null>(null);
  const [dragOverToneFolderId, setDragOverToneFolderId] = useState<
    string | null
  >(null);
  const [isDragging, setIsDragging] = useState(false);
  const [isDraggingTone, setIsDraggingTone] = useState(false);
  const editInputRef = useRef<HTMLInputElement>(null);
  const editToneInputRef = useRef<HTMLInputElement>(null);
  const justStartedEditingRef = useRef(false);
  const justStartedEditingToneRef = useRef(false);
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div className="flex flex-col items-center text-center p-4 rounded-lg bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700">
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                  {t(
                                    'Type / to instantly access any saved prompt',
                                  )}
                                </p>
                              </div>
...
- Professional closing`}
                      ...
                      <div
                        className={
                          !selectedPromptId ? 'space-y-2' : 'space-y-1'
                        }
                      >
                        {/* Prompt Folders */}
          ...
          <div className="flex-1 flex items-center justify-center p-12">
            <div className="text-center max-w-md">
              ...
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                Tones Feature Coming Soon
              </h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Create custom voice profiles by uploading content or pasting
                text. Train the AI to match specific writing styles and tones.
              </p>
            </div>
          </div>
        )}
