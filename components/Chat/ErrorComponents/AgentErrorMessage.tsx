...
const ErrorDetails: React.FC<ErrorDetailsProps> = ({ error, expanded, onToggle }) => {
  const { t } = useTranslation('errors');
  ...
  return (
    <div className="mt-3">
      ...
      {expanded && (
        <div id="error-details" className="mt-2 p-3 bg-gray-100 dark:bg-gray-800 rounded-md text-sm">
          <div className="space-y-2">
            <div>
              <span className="font-medium text-gray-700 dark:text-gray-300">Error ID:</span>
              <span className="ml-2 font-mono text-gray-600 dark:text-gray-400">{error.id}</span>
            </div>
            <div>
              <span className="font-medium text-gray-700 dark:text-gray-300">Agent Type:</span>
              <span className="ml-2 text-gray-600 dark:text-gray-400">{error.agentType}</span>
            </div>
            <div>
              <span className="font-medium text-gray-700 dark:text-gray-300">Category:</span>
              <span className="ml-2 text-gray-600 dark:text-gray-400">{error.category}</span>
            </div>
            <div>
              <span className="font-medium text-gray-700 dark:text-gray-300">Code:</span>
              <span className="ml-2 font-mono text-gray-600 dark:text-gray-400">{error.code}</span>
            </div>
            <div>
            ...
            {error.retryCount !== undefined && (
              <div>
                <span className="font-medium text-gray-700 dark:text-gray-300">Retry Count:</span>
                <span className="ml-2 text-gray-600 dark:text-gray-400">{error.retryCount}</span>
              </div>
            )}
            {error.details && (
              <div>
...
export const AgentErrorMessage: React.FC<AgentErrorMessageProps> = ({
  ...
  const getHelpMessage = () => {
    const helpMessages: Record<string, string> = {
      [AgentErrorCategory.NETWORK]: t('Check your internet connection and try again.'),
      [AgentErrorCategory.AUTHENTICATION]: t('Please check your API keys in settings.'),
      [AgentErrorCategory.RATE_LIMIT]: t('You\'ve reached the usage limit. Please wait and try again.'),
      [AgentErrorCategory.VALIDATION]: t('Please check your input and try again.'),
      [AgentErrorCategory.TIMEOUT]: t('The operation took too long. Try with a simpler request.'),
      [AgentErrorCategory.SERVICE_UNAVAILABLE]: t('The service is temporarily unavailable.'),
    };

    return helpMessages[error.category] || t('Please try again or contact support if the problem persists.');
  };
      ...
      >
        ...
        {error.isRecoverable && onRetry && (
          <button
            ...
            aria-label={t('Retry')}
          >
            <IconRefresh className={`h-4 w-4 ${actionLoading ? 'animate-spin' : ''} ${styles.icon}`} />
          </button>
        )}
        {onDismiss && (
          <button
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <CategoryIcon className={`h-5 w-5 mt-0.5 flex-shrink-0 ${styles.icon}`} />
        
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <div>
              <h3 className={`text-sm font-medium ${styles.text}`}>
                {t(`errorCategories.${error.category}`, { defaultValue: error.category })}
              </h3>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {error.isRecoverable && error.recoveryStrategy === RecoveryStrategy.RETRY && onRetry && (
              <RecoveryAction
            ...
            {error.recoveryStrategy === RecoveryStrategy.FALLBACK && onFallback && (
              <RecoveryAction
            ...
            {error.recoveryStrategy === RecoveryStrategy.ALTERNATIVE_AGENT && onFallback && (
              <RecoveryAction
            ...
            {error.recoveryStrategy === RecoveryStrategy.USER_ACTION && onUserAction && (
              <RecoveryAction
