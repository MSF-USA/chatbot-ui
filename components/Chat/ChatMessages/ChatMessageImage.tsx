...
const ChatMessageImage: FC<ChatMessageImageProps> = ({
  ...
  const defaultImageStyleProps: ImageStyleProps = {
    maxHeight: '20rem',
    maxWidth: '30rem',
  };
  ...
  const [imageStyleProps, setImageStyleProps] = useState<ImageStyleProps>(
    defaultImageStyleProps,
  );
    ...
    (content as Array<TextMessageContent | ImageMessageContent>)
      ...
      .forEach((contentMessage) => {
        if (contentMessage.type === 'image_url') {
          ...
          throw new Error(
            `Unexpected message type for message: ${JSON.stringify(
              contentMessage,
              null,
              2,
            )}`,
