...
describe('Test Environment Variables', () => {
    ...
    let prompt = process.env.NEXT_PUBLIC_DEFAULT_SYSTEM_PROMPT ||
...
describe('findWorkingConfiguration Function', () => {
    ...
    beforeEach(() => {
        ...
        global.fetch = vi.fn().mockImplementation(() => Promise.resolve(
            mockResponse(200, "OK", { data: 'valid response' })
        ));
    })
    ...
    it('Throws error when no valid configuration found', async () => {
        ...
        await expect(findWorkingConfiguration(key)).rejects.toThrow('No valid configuration found for this key');
    });
