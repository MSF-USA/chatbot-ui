import { savePrompts, updatePrompt } from '@/utils/app/prompts';
...
describe('prompts tests', () => {
  ...
  describe('updatePrompt function', () => {
    it('should update prompt', () => {
      ...
      // @ts-ignore
      expect(
        (result.all as Prompt[]).find((p) => p.id === updatedPrompt.id).content,
      ).toBe(newPromptText);
    });
    ...
    it('should not update a non-existent prompt', () => {
      ...
      expect(
        (result.all as Prompt[]).find((p) => p.id === nonExistentPrompt.id),
      ).toBeUndefined();
    });
  });
