...
import { SimpleFeatureFlagService as FeatureFlagService, UserContext, getFeatureFlagService } from './simpleFeatureFlags';
import { AgentPoolingService, getAgentPoolingService } from './agentPoolingService';
import { AgentFactory } from './agentFactory';
import { IntentAnalysisService, getIntentAnalysisService } from './intentAnalysis';
...
export class EnhancedChatService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      try {
        ...
        console.warn('[WARN] Agent pooling service not available, will initialize on demand');
      }
      ...
      console.error('[ERROR] Failed to initialize Enhanced Chat Service:', error);
      throw error;
    }
  }
   ...
   */
  async handleRequest(req: NextRequest, providedSession?: Session | null, providedToken?: JWT | null, parsedBody?: any): Promise<Response> {
    const startTime = Date.now();
    this.metrics.totalRequests++;

    try {
      if (!this.isInitialized) {
        console.warn('[WARN] Enhanced Chat Service not initialized, using standard chat fallback');
        ...
        if (parsedBody) {
          ...
          return this.standardChatService.handleRequest(fallbackRequest as NextRequest, providedSession, providedToken);
        }
        return this.standardChatService.handleRequest(req, providedSession, providedToken);
      }

      // Parse request (use provided body if available to avoid double consumption)
      const chatRequest = parsedBody ? this.parseRequestFromBody(parsedBody) : await this.parseRequest(req);
      const userContext = await this.extractUserContext(req, providedSession, providedToken);

      // Make routing decision
      const routingDecision = await this.makeRoutingDecision(chatRequest, userContext, providedSession);
      ...
      if (routingDecision.shouldUseAgents) {
        ...
        response = await this.handleWithAgents(chatRequest, userContext, routingDecision, shouldStream, providedSession, providedToken);
        this.metrics.agentRequests++;
      } else {
        // Use standard chat system
        response = await this.handleWithStandardChat(req, providedSession, providedToken, chatRequest);
      ...
      try {
        ...
        if (parsedBody) {
          ...
          fallbackResponse = await this.standardChatService.handleRequest(fallbackRequest as NextRequest, providedSession, providedToken);
        } else {
          fallbackResponse = await this.standardChatService.handleRequest(req, providedSession, providedToken);
        }
  ...
  private async makeRoutingDecision(
    ...
    try {
      // Get feature flags
      const flags = await this.featureFlagService.getAgentRoutingFlags(userContext);
      ...
      const shouldRoute = await this.featureFlagService.shouldRouteToAgents(userContext);
      if (!shouldRoute) {
        return {
          shouldUseAgents: false,
          agentType: AgentType.WEB_SEARCH, // Default to web-search when not using agents
      ...
      const intentContext: IntentAnalysisContext = {
        ...
        conversationHistory: chatRequest.messages.slice(0, -1).map(msg => this.extractMessageText(msg)),
  ...
  private async handleWithAgents(
    ...
    try {
      ...
      if (!this.agentPoolingService) {
        console.log('[INFO] Agent pooling not available, creating agent directly');
        return this.handleWithDirectAgent(chatRequest, userContext, routingDecision, shouldStream, providedSession, providedToken);
      }
      ...
      if (shouldStream) {
        ...
        const pooledAgent = await this.agentPoolingService.getAgent(routingDecision.agentType);
        if (!pooledAgent) {
          throw new Error(`No agent available for type: ${routingDecision.agentType}`);
        }
        
        const stream = await (pooledAgent.instance as BaseAgent).executeStreaming(agentExecutionContext);
        ...
        return await this.formatAgentResponse(executionResult.response, routingDecision, chatRequest, providedSession, providedToken);
      }

    } catch (error) {
      console.error(`[ERROR] Agent execution failed for ${routingDecision.agentType}:`, error);
      
      // Check if fallback is enabled
      if (routingDecision.flags.fallbackOnError !== false) {
        console.log('[INFO] Agent failed, falling back to standard chat system');
        return this.handleFallbackToStandardChat(chatRequest, providedSession, providedToken);
      }
  ...
  private async handleWithDirectAgent(
    ...
    try {
      ...
      if (shouldStream) {
        ...
        const stream = await (agent as BaseAgent).executeStreaming(agentExecutionContext);
        return this.formatStreamingResponse(stream, routingDecision);
      } else {
        // Execute request (non-streaming)
        const result = await (agent as BaseAgent).execute(agentExecutionContext);
        return await this.formatAgentResponse(result, routingDecision, chatRequest, providedSession, providedToken);
      }
   ...
   */
  private async handleFallbackToStandardChat(chatRequest: EnhancedChatRequest, providedSession?: Session | null, providedToken?: JWT | null): Promise<Response> {
    ...
    return this.standardChatService.handleRequest(originalRequest as NextRequest, providedSession, providedToken);
  }

  /**
   * Handle request with standard chat system
   */
  private async handleWithStandardChat(req: NextRequest, providedSession?: Session | null, providedToken?: JWT | null, chatRequest?: EnhancedChatRequest): Promise<Response> {
    ...
    if (chatRequest) {
      ...
      return this.standardChatService.handleRequest(standardRequest as NextRequest, providedSession, providedToken);
    }
    return this.standardChatService.handleRequest(req, providedSession, providedToken);
  }
  ...
  private async formatAgentResponse(
    ...
    if (result.structuredContent && result.structuredContent.type === 'url_content') {
      console.log('[INFO] [URLAgent] Detected structured content, processing through AI...', {
      ...
      return await this.processStructuredContent(result, routingDecision, chatRequest, providedSession, providedToken);
    }
  ...
  private async processStructuredContent(
    ...
    try {
      const systemMessage = messagesWithContent.find(m => m.role === 'system');
      const systemContent = systemMessage ? 
        (typeof systemMessage.content === 'string' ? systemMessage.content.substring(0, 200) + '...' : '[Complex content]') 
        : 'None';
      
      console.log('[DEBUG] [URLAgent] Calling standard chat service with messages:', {
        messageCount: messagesWithContent.length,
        lastMessage: messagesWithContent[messagesWithContent.length - 1]?.role,
      ...
      console.log('[DEBUG] [URLAgent] Making request to standard chat service...');
      const standardResponse = await this.standardChatService.handleRequest(
        standardRequest as NextRequest,
        providedSession,
        providedToken
      );
      console.log('[DEBUG] [URLAgent] Standard chat service response received:', {
      ...
      if (standardResponse.headers.get('Content-Type')?.includes('text/event-stream')) {
  ...
  private async extractUserContext(req: NextRequest, providedSession?: Session | null, providedToken?: JWT | null): Promise<UserContext> {
    ...
    if (providedSession?.user && userId === 'anonymous') {
      userId = providedSession.user.id || (providedSession.user as any).email || 'authenticated_user';
  ...
  private selectAgentType(intentAnalysis: any, flags: any): AgentType {
    ...
    if (flags.agentType === 'legacy') {
      console.log('[EnhancedChatService] Forced to legacy mode, defaulting to WEB_SEARCH');
      return AgentType.WEB_SEARCH;
    }
    
    // Use the recommended agent type from AI-powered intent analysis
    const selectedType = intentAnalysis.recommendedAgent || AgentType.WEB_SEARCH;
  ...
  getMetrics() {
    return {
      ...this.metrics,
      agentUsagePercentage: this.metrics.totalRequests > 0 
  ...
  private getValidOpenAIInstance(): AzureOpenAI | undefined {
    ...
    try {
      console.log('[INFO] Creating Azure OpenAI instance on-demand for intent analysis');
    ...
    console.warn('[WARN] No valid OpenAI instance available, intent analysis will use heuristic fallback');
    return undefined;
  }
  ...
  private initializeAIServices(modelId?: string): void {
    try {
      ...
      console.log('[INFO] Azure OpenAI initialized for intent analysis with Azure AD credentials, model:', this.modelId); 
 
    } catch (error) {
      console.error('[ERROR] Failed to initialize Azure OpenAI with Azure AD credentials:', error);
      // Ensure we have a fallback model ID even if initialization fails
      this.modelId = 'gpt-4o-mini';
      console.warn('[WARN] Azure OpenAI initialization failed, intent analysis will use heuristic fallback only');
      console.log('[INFO] Using fallback model ID after initialization error:', this.modelId);
    }
  }
  ...
  async shutdown(): Promise<void> {
    try {
      ...
      console.error('[ERROR] Error during Enhanced Chat Service shutdown:', error);
    }
  }
}
