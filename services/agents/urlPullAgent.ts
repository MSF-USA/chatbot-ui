...
export class UrlPullAgent extends BaseAgent {
  ...
  private cache: Map<string, { data: ProcessedUrl; timestamp: number }> =
    new Map();
  ...
  protected validateSpecificConfig(): string[] {
    ...
    if (
      this.urlPullConfig.concurrencyLimit &&
      this.urlPullConfig.concurrencyLimit <= 0
    ) {
  ...
  private parseRequest(query: string): UrlPullRequest {
    ...
    return {
      ...
      config: {
        ...
        timeout:
          this.urlPullConfig.processingTimeout ||
          DEFAULT_URL_PULL_CONFIG.timeout,
        concurrencyLimit:
          this.urlPullConfig.concurrencyLimit ||
          DEFAULT_URL_PULL_CONFIG.concurrencyLimit,
        enableCaching:
          this.urlPullConfig.enableCaching ??
          DEFAULT_URL_PULL_CONFIG.enableCaching,
        cacheTtl: this.urlPullConfig.cacheTtl
          ? this.urlPullConfig.cacheTtl * 1000
          : DEFAULT_URL_PULL_CONFIG.cacheTtl,
      },
      context: {
        enableParallelProcessing:
          this.urlPullConfig.enableParallelProcessing ?? true,
        enableContentExtraction:
          this.urlPullConfig.enableContentExtraction ?? true,
  ...
  private async processUrls(
    request: UrlPullRequest,
    context: AgentExecutionContext,
  ): Promise<UrlPullResponse> {
    ...
    if (urls.length === 0) {
      throw new UrlPullError(
        'No valid URLs found to process',
        UrlPullErrorType.INVALID_URL,
      );
    }
    ...
    if (urls.length > config.maxUrls) {
      throw new UrlPullError(
        `Too many URLs: ${urls.length} exceeds limit of ${
          config.maxUrls || DEFAULT_URL_PULL_CONFIG.maxUrls
        }`,
        UrlPullErrorType.CONTENT_ERROR,
      );
    ...
    const processingMethod = request.context?.enableParallelProcessing
      ? 'parallel'
      : 'sequential';
    const concurrencyLimit = Math.min(
      config.concurrencyLimit || DEFAULT_URL_PULL_CONFIG.concurrencyLimit,
      urls.length,
    );
    ...
    return {
      ...
      processingStats: {
        ...
        averageProcessingTime:
          urls.length > 0 ? processingTime / urls.length : 0,
      },
  ...
  private async processUrlsParallel(
    urls: string[],
    config: any,
    ...
    const urlPromises = urls.map(async (url) => {
      ...
      try {
        ...
        if (result.type === 'success') {
          ...
          if (result.data.retryAttempts)
            retryCount += result.data.retryAttempts;
        }
      } finally {
  ...
  private async processUrlsSequential(
    urls: string[],
    config: any,
  ): Promise<ProcessingResult> {
    const processedUrls: ProcessedUrl[] = [];
  ...
  private async processUrl(
    ...
    | { type: 'success'; data: ProcessedUrl }
    | { type: 'failure'; data: FailedUrl }
  > {
    const startTime = Date.now();
  ...
  private calculateConfidence(response: UrlPullResponse): number {
    const successRate =
      response.processingStats.totalUrls > 0
        ? response.processingStats.successfulUrls /
          response.processingStats.totalUrls
        : 0;
  ...
  private formatSummaryResponse(response: UrlPullResponse): string {
    ...
    summary += `. Total content extracted: ${this.formatBytes(
      processingStats.totalContentSize,
    )}.`;
