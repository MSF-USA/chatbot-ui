...
const DEFAULT_PARAMETER_CONFIG: ParameterExtractionConfig = {
  [AgentType.WEB_SEARCH]: {
    queryKeywords: [
      'search', 'find', 'look up', 'google', 'research', 'investigate',
    ...
    countIndicators: [
      'top 5', 'first 10', 'best 3', 'top 10', 'first 5',
      'show me 5', 'give me 3', 'list 10', 'find 5'
    ],
    marketIndicators: {
      'us': 'en-US',
      'usa': 'en-US',
  ...
  [AgentType.CODE_INTERPRETER]: {
    ...
    fileUploadIndicators: [
      'upload', 'file', 'csv', 'json', 'data', 'dataset',
      'spreadsheet', 'excel', 'txt', 'log', 'image', 'picture'
    ],
  },
  [AgentType.URL_PULL]: {
    urlPatterns: [
  ...
  [AgentType.LOCAL_KNOWLEDGE]: {
    companyKeywords: [
      'company', 'our', 'internal', 'organization', 'org',
      'corporate', 'business', 'enterprise', 'firm'
    ],
    internalTopics: [
      'policy', 'procedure', 'guideline', 'handbook', 'manual',
      'process', 'workflow', 'protocol', 'standard', 'rule'
    ],
    knowledgeBaseIndicators: [
      'documentation', 'docs', 'wiki', 'knowledge base',
      'help center', 'support', 'faq', 'guide', 'tutorial'
    ],
  },
...
interface ExtractedCodeParams {
  ...
  libraries?: string[];
  file_upload?: boolean;
  execution_type?: 'analysis' | 'visualization' | 'calculation' | 'debugging' | 'tutorial';
}
...
interface ExtractedLocalKnowledgeParams {
  topics: string[];
  keywords: string[];
  category?: 'policy' | 'procedure' | 'contact' | 'product' | 'training' | 'general';
  department?: string;
}
...
export class ParameterExtractionEngine {
   ...
   */
  public static getInstance(config?: Partial<ParameterExtractionConfig>): ParameterExtractionEngine {
    if (!ParameterExtractionEngine.instance) {
      ParameterExtractionEngine.instance = new ParameterExtractionEngine(config);
    }
  ...
  public extractParameters(
    ...
    try {
      ...
      const ruleBasedParams = this.extractRuleBasedParameters(agentType, query, context);
      extractedParams = { ...extractedParams, ...ruleBasedParams };
      
      // Apply post-processing and validation
      extractedParams = this.postProcessParameters(agentType, extractedParams, query);
  ...
  public extractFromAIResponse(
    ...
    const aiParams = aiResponse.parameters?.[agentKey as keyof typeof aiResponse.parameters] || aiResponse.parameters || {};
    
    return this.extractParameters(agentType, context?.query || '', context, aiParams);
  }

  /**
   * Validate extracted parameters for an agent type
   */
  public validateParameters(agentType: AgentType, parameters: Record<string, any>): {
    valid: boolean;
    errors: string[];
    warnings: string[];
    ...
    try {
      switch (agentType) {
        ...
        case AgentType.LOCAL_KNOWLEDGE:
          this.validateLocalKnowledgeParams(sanitizedParameters, errors, warnings);
          break;
   ...
   */
  public getDefaultParameters(agentType: AgentType, query?: string): Record<string, any> {
  ...
  private processAIParameters(agentType: AgentType, aiParameters: any): Record<string, any> {
  ...
  private extractWebSearchParams(query: string, context?: IntentAnalysisContext): ExtractedWebSearchParams {
    ...
    for (const [indicator, freshness] of Object.entries(this.config[AgentType.WEB_SEARCH]?.freshnessIndicators || {})) {
    ...
    // Extract count
    for (const countIndicator of this.config[AgentType.WEB_SEARCH]?.countIndicators || []) {
    ...
    for (const [indicator, market] of Object.entries(this.config[AgentType.WEB_SEARCH]?.marketIndicators || {})) {
      if (queryLower.includes(indicator)) {
  ...
  private extractCodeParams(query: string, context?: IntentAnalysisContext): ExtractedCodeParams {
    const params: ExtractedCodeParams = {};

    // Extract language
    const queryLower = query.toLowerCase();
    for (const [indicator, language] of Object.entries(this.config[AgentType.CODE_INTERPRETER]?.languageIndicators || {})) {
      if (queryLower.includes(indicator)) {
    ...
    for (const [indicator, libs] of Object.entries(this.config[AgentType.CODE_INTERPRETER]?.libraryIndicators || {})) {
      if (queryLower.includes(indicator)) {
        libraries.push(...libs);
      }
    }
    ...
    const fileIndicators = this.config[AgentType.CODE_INTERPRETER]?.fileUploadIndicators || [];
    params.file_upload = fileIndicators.some(indicator => queryLower.includes(indicator));
  ...
  private extractUrlParams(query: string, context?: IntentAnalysisContext): ExtractedUrlParams {
    ...
    const queryLower = query.toLowerCase();
    for (const [indicator, type] of Object.entries(this.config[AgentType.URL_PULL]?.analysisTypeIndicators || {})) {
      if (queryLower.includes(indicator)) {
    ...
    } else if (queryLower.includes('detailed') || queryLower.includes('complete')) {
      params.output_format = 'detailed';
    } else if (queryLower.includes('json') || queryLower.includes('structured')) {
  ...
  private extractLocalKnowledgeParams(query: string, context?: IntentAnalysisContext): ExtractedLocalKnowledgeParams {
    ...
    // Extract topics
    const topics: string[] = [];
    for (const topic of this.config[AgentType.LOCAL_KNOWLEDGE]?.internalTopics || []) {
    ...
    } else if (queryLower.includes('procedure') || queryLower.includes('process')) {
    ...
    } else if (queryLower.includes('product') || queryLower.includes('service')) {
      params.category = 'product';
    } else if (queryLower.includes('training') || queryLower.includes('tutorial')) {
      params.category = 'training';
    }

    // Extract department
    const departments = ['hr', 'engineering', 'sales', 'marketing', 'finance', 'legal', 'it', 'support'];
  ...
  private extractThirdPartyParams(query: string, context?: IntentAnalysisContext): ExtractedThirdPartyParams {
    ...
    // Extract service
    for (const [indicator, service] of Object.entries(this.config[AgentType.THIRD_PARTY]?.serviceIndicators || {})) {
      if (queryLower.includes(indicator)) {
    ...
    // Extract action
    if (queryLower.includes('create') || queryLower.includes('make') || queryLower.includes('add')) {
    ...
    // Extract endpoint
    for (const [indicator, endpoint] of Object.entries(this.config[AgentType.THIRD_PARTY]?.endpointPatterns || {})) {
      if (queryLower.includes(indicator)) {
  ...
  private postProcessParameters(
    ...
    switch (agentType) {
      ...
      case AgentType.URL_PULL:
        ...
        if (processed.urls) {
          processed.urls = processed.urls.filter((url: string) => this.isValidUrl(url));
        }
        break;
    }
  ...
  private determineSearchCategory(query: string): 'news' | 'academic' | 'shopping' | 'images' | 'videos' | 'general' {
    const queryLower = query.toLowerCase();
    
    if (queryLower.includes('news') || queryLower.includes('breaking') || queryLower.includes('headline')) {
      return 'news';
    }
    if (queryLower.includes('research') || queryLower.includes('study') || queryLower.includes('paper')) {
      return 'academic';
    }
    if (queryLower.includes('buy') || queryLower.includes('price') || queryLower.includes('shop')) {
      return 'shopping';
    }
    if (queryLower.includes('image') || queryLower.includes('picture') || queryLower.includes('photo')) {
      return 'images';
    }
    if (queryLower.includes('video') || queryLower.includes('watch') || queryLower.includes('clip')) {
      return 'videos';
    }
  ...
  private determineExecutionType(query: string): 'analysis' | 'visualization' | 'calculation' | 'debugging' | 'tutorial' {
    const queryLower = query.toLowerCase();
    
    if (queryLower.includes('debug') || queryLower.includes('error') || queryLower.includes('fix')) {
      return 'debugging';
    }
    if (queryLower.includes('plot') || queryLower.includes('chart') || queryLower.includes('graph') || queryLower.includes('visualize')) {
      return 'visualization';
    }
    if (queryLower.includes('calculate') || queryLower.includes('compute') || queryLower.includes('math')) {
      return 'calculation';
    }
    if (queryLower.includes('tutorial') || queryLower.includes('learn') || queryLower.includes('example')) {
      return 'tutorial';
    }
  ...
  private isTimeSensitive(query: string): boolean {
    const timeSensitiveWords = ['today', 'now', 'current', 'latest', 'recent', 'breaking', 'live', 'real-time'];
    const queryLower = query.toLowerCase();
  ...
  private inferLanguageFromContext(query: string): string {
    ...
    if (query.includes('def ') || query.includes('import ') || query.includes('print(')) {
      return 'python';
    }
    if (query.includes('function ') || query.includes('console.log') || query.includes('const ')) {
      return 'javascript';
    }
    if (query.includes('interface ') || query.includes(': string') || query.includes(': number')) {
      return 'typescript';
    }
    if (query.includes('echo ') || query.includes('ls ') || query.includes('grep ')) {
      return 'bash';
    }
  ...
  private validateWebSearchParams(params: Record<string, any>, errors: string[], warnings: string[]): void {
    if (!params.query || typeof params.query !== 'string' || params.query.trim() === '') {
      errors.push('Search query is required and must be a non-empty string');
    }
    
    if (params.freshness && !['day', 'week', 'month', 'year'].includes(params.freshness)) {
      errors.push('Freshness must be one of: day, week, month, year');
    }
    
    if (params.count && (typeof params.count !== 'number' || params.count < 1 || params.count > 50)) {
      warnings.push('Count should be between 1 and 50');
      params.count = Math.max(1, Math.min(50, params.count || 10));
    }
  }

  private validateCodeParams(params: Record<string, any>, errors: string[], warnings: string[]): void {
    const supportedLanguages = ['python', 'javascript', 'typescript', 'bash', 'sql', 'r'];
  ...
  private validateUrlParams(params: Record<string, any>, errors: string[], warnings: string[]): void {
    ...
    if (params.analysis_type && !['content', 'metadata', 'full', 'comparison', 'extraction'].includes(params.analysis_type)) {
      errors.push('Analysis type must be one of: content, metadata, full, comparison, extraction');
    }
  }

  private validateLocalKnowledgeParams(params: Record<string, any>, errors: string[], warnings: string[]): void {
    ...
    if ((!params.topics || params.topics.length === 0) && (!params.keywords || params.keywords.length === 0)) {
      warnings.push('No topics or keywords specified - search may be too broad');
    }
  }

  private validateThirdPartyParams(params: Record<string, any>, errors: string[], warnings: string[]): void {
    if (!params.service || typeof params.service !== 'string') {
      errors.push('Service name is required');
    }
    
    if (!params.action || !['create', 'read', 'update', 'delete', 'list', 'search'].includes(params.action)) {
      errors.push('Action must be one of: create, read, update, delete, list, search');
    }
  }
}
 ...
 */
export function getParameterExtractionEngine(config?: Partial<ParameterExtractionConfig>): ParameterExtractionEngine {
