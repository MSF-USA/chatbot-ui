...
export class IntentAnalysisService {
  ...
  public static getInstance(
    config?: Partial<IntentAnalysisConfig>,
  ...
  public async analyzeIntent(
    ...
    console.log(
      '[IntentAnalysis] Starting intent analysis for query:',
    ...
    console.log(
      '[IntentAnalysis] Detected language:',
      detectedLanguage,
      'Using locale:',
      analysisLocale,
    );
    ...
    try {
      ...
      if (!validation.valid) {
        console.log(
          '[IntentAnalysis] Input validation failed:',
          validation.errors,
        );
        throw new Error(
      ...
      if (this.config.enableCaching) {
        ...
        console.log(
          '[IntentAnalysis] Caching disabled, proceeding with analysis',
        );
      }
      ...
      if (this.config.enableAIClassification && openai && modelId && user) {
        console.log(
          '[IntentAnalysis] AI classification enabled and dependencies available',
        );
        ...
        try {
          ...
          console.log(
            '[IntentAnalysis] AI classification completed successfully',
            {
          ...
          if (aiResult.confidence >= this.config.aiConfidenceThreshold) {
            console.log(
              '[IntentAnalysis] AI confidence meets threshold, using AI classification result',
            );
            ...
            result.confidence = this.calculateAdvancedConfidence(
              result,
              context,
              aiResult,
            );
            console.log(
              '[IntentAnalysis] Recalculated confidence score:',
              result.confidence,
            );
            this.updateMetrics('ai_success', Date.now() - startTime);
          } else {
            console.log(
              '[IntentAnalysis] AI confidence too low, falling back to heuristic analysis',
              {
            ...
            result = await this.performHeuristicFallback(
              context,
              analysisLocale,
              startTime,
            );
          ...
          console.warn(
            '[WARNING] AI classification failed, falling back to heuristics',
            {
  ...
  private async performAIClassification(
    ...
    console.log(
      '[IntentAnalysis] Messages prepared, system prompt length:',
      this.getSystemPrompt(locale).length,
    );

    console.log(
      '[IntentAnalysis] Sending request to AI model for classification',
      {
        modelId,
        temperature: 0.3,
        maxTokens: 800,
    ...
    try {
      const response =
        await getStructuredResponse<IntentClassificationResponse>(
          openai,
          messages,
          modelId,
          user,
  ...
  private async performHeuristicFallback(
    ...
    if (!this.config.enableHeuristicFallback) {
      console.log(
        '[IntentAnalysis] Heuristic fallback disabled, returning default fallback result',
      );
      return this.createFallbackResult(context, locale, Date.now() - startTime);
    }

    console.log(
      '[IntentAnalysis] Performing heuristic analysis on query:',
      context.query.substring(0, 100),
    );
    const heuristicResult = this.performHeuristicAnalysis(
      context.query,
      locale,
    );
    ...
    console.log(
      '[IntentAnalysis] Building final result from heuristic analysis',
    );

    return this.buildResultFromHeuristic(
      heuristicResult,
      locale,
      startTime,
      context,
  ...
  private performHeuristicAnalysis(
    ...
    console.log(
      '[IntentAnalysis] Starting heuristic analysis for query:',
    ...
    console.log(
      '[IntentAnalysis] Initial default match:',
      bestMatch.agentType,
      'with confidence:',
    ...
    for (const [agentType, guidance] of Object.entries(
      ...
      console.log('[IntentAnalysis] Agent', agentType, 'analysis result:', {
        ...
        matchedPatterns:
          result.matchedPatterns.length > 0
            ? result.matchedPatterns.slice(0, 3)
            : 'none',
      ...
      if (result.confidence > bestMatch.confidence) {
        console.log(
          '[IntentAnalysis] Found better match:',
          agentType,
          'with confidence:',
          result.confidence,
          '(previous best:',
    ...
    console.log(
      '[IntentAnalysis] Heuristic analysis complete. Best match:',
      bestMatch.agentType,
      'with confidence:',
      bestMatch.confidence,
    );
  ...
  private analyzeAgentMatch(
    ...
    // Pattern matching
    console.log(
      `[IntentAnalysis] Performing pattern matching for ${agentType}`,
    );
    ...
    console.log(
      `[IntentAnalysis] Pattern matching complete for ${agentType}, confidence: ${confidence}`,
    );

    // Keyword matching
    console.log(
      `[IntentAnalysis] Performing keyword matching for ${agentType}`,
    );
    ...
    parameters = this.extractHeuristicParameters(
      query,
      agentType,
      matchedPatterns,
    );
    console.log(
      `[IntentAnalysis] Parameter extraction complete for ${agentType}`,
      {
    ...
    console.log(
      `[IntentAnalysis] Applying agent-specific boosts for ${agentType}, base confidence: ${confidence}`,
    );
    const boostedConfidence = this.applyAgentSpecificBoosts(
      query,
      agentType,
      confidence,
    );
    if (boostedConfidence > confidence) {
      console.log(
        `[IntentAnalysis] Applied confidence boost for ${agentType}:`,
        {
          before: confidence,
    ...
    console.log(
      `[IntentAnalysis] Checking for user exclusions for ${agentType}`,
    );
    const excludedConfidence = this.applyUserExclusions(
      query,
      agentType,
      confidence,
    );
    if (excludedConfidence < confidence) {
      console.log(
        `[IntentAnalysis] Applied user exclusion penalty for ${agentType}:`,
        {
          before: confidence,
    ...
    console.log(
      `[IntentAnalysis] Final confidence for ${agentType}: ${finalConfidence}`,
    );
  ...
  private applyAgentSpecificBoosts(
    query: string,
    agentType: AgentType,
    baseConfidence: number,
  ): number {
    console.log(
      `[IntentAnalysis] Applying agent-specific boosts for ${agentType}, starting confidence: ${baseConfidence}`,
    );
    ...
    switch (agentType) {
      case AgentType.URL_PULL:
        ...
        if (query.match(/https?:\/\/[^\s]+/)) {
          ...
          console.log(
            `[IntentAnalysis] URL detected, boosting confidence from ${oldConfidence} to ${confidence}`,
          );
        }
        break;
      ...
      case AgentType.CODE_INTERPRETER:
        console.log(
          `[IntentAnalysis] Checking code block patterns for ${agentType}`,
        );
        ...
        if (query.match(/```[\s\S]*?```/)) {
          ...
          console.log(
            `[IntentAnalysis] Code block detected, boosting confidence from ${oldConfidence} to ${confidence}`,
          );
        }
        break;
      ...
      case AgentType.WEB_SEARCH:
        console.log(
          `[IntentAnalysis] Checking web search specific patterns for ${agentType}`,
        );

        // High boost for time-sensitive queries
        if (
          query.match(
            /\b(breaking|urgent|immediate|now|today|latest|current|recent)\b/i,
          )
        ) {
          const boost = 0.2;
          console.log(
            `[IntentAnalysis] Time-sensitive query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          query.match(
            /\b(compare|vs|versus|reviews?|ratings?|better than|difference between)\b/i,
          )
        ) {
          const boost = 0.15;
          console.log(
            `[IntentAnalysis] Comparison/review query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (query.match(/\b(trending|viral|popular|top \d+|best \d+)\b/i)) {
          const boost = 0.15;
          console.log(
            `[IntentAnalysis] Trending/popular content query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          query.match(
            /\b(price|stock|weather|score|market|election|poll|results)\b/i,
          )
        ) {
          const boost = 0.15;
          console.log(
            `[IntentAnalysis] Real-time data query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          query.match(
            /\b(this (week|month|year)|in \d{4}|since \d{4}|yesterday|recently)\b/i,
          )
        ) {
          const boost = 0.1;
          console.log(
            `[IntentAnalysis] Temporal reference detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        break;
      ...
      case AgentType.LOCAL_KNOWLEDGE:
        console.log(
          `[IntentAnalysis] Checking local knowledge specific patterns for ${agentType}`,
        );

        // Boost for company-specific language
        if (
          query.match(
            /\b(our|company|internal|organization|corporate|enterprise)\b/i,
          )
        ) {
          const boost = 0.2;
          console.log(
            `[IntentAnalysis] Company-specific language detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          query.match(
            /\b(documentation|docs|manual|handbook|policy|procedure|guideline|process|workflow)\b/i,
          )
        ) {
          const boost = 0.25;
          console.log(
            `[IntentAnalysis] Documentation/knowledge query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          ...
          const boost = 0.2;
          console.log(
            `[IntentAnalysis] FAQ/support query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          ...
          const boost = 0.15;
          console.log(
            `[IntentAnalysis] Department/role query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          query.match(
            /\b(knowledge base|wiki|internal docs|company info|organizational|protocols)\b/i,
          )
        ) {
          const boost = 0.3;
          console.log(
            `[IntentAnalysis] Knowledge base specific terms detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        ...
        if (
          ...
          const boost = 0.15;
          console.log(
            `[IntentAnalysis] Employee-specific query detected, adding boost of ${boost}`,
          );
          confidence += boost;
        }
        break;
    ...
    console.log(
      `[IntentAnalysis] Final confidence after agent-specific boosts for ${agentType}: ${confidence}`,
    );
    return confidence;
  }
  ...
  private getMatchMethod(
    ...
    if (
      matchedPatterns.some((p) =>
        ['today', 'recent', 'latest', 'breaking'].includes(p.toLowerCase()),
      )
    ) {
  ...
  private buildResult(
    ...
    console.log(
      '[IntentAnalysis] Mapped agent type string to enum:',
      aiResult.agent_type,
      '->',
      agentType,
    );
    ...
    const parameters = this.extractParametersFromAI(
      aiResult.parameters,
      agentType,
      context,
    );
    console.log(
      '[IntentAnalysis] Parameters extracted:',
  ...
  private buildResultFromHeuristic(
    ...
    const reasoning = `Heuristic analysis detected ${
      heuristicResult.method
    } patterns: ${heuristicResult.matchedPatterns.join(', ')}`;
    console.log(
      '[IntentAnalysis] Heuristic reasoning:',
      reasoning.substring(0, 100) + (reasoning.length > 100 ? '...' : ''),
    );
    ...
    if (context) {
      console.log(
        '[IntentAnalysis] Applying advanced confidence scoring to heuristic result',
      );
  ...
  private createFallbackResult(
    ...
    console.log(
      '[IntentAnalysis] Creating fallback result due to analysis failure or low confidence',
    );
    console.log(
      '[IntentAnalysis] Using default agent:',
    ...
    const result = {
      ...
      parameters: {},
      reasoning:
        'Fallback to default agent due to analysis failure or low confidence',
  ...
  private calculateAdvancedConfidence(
    ...
    console.log(
      '[IntentAnalysis] Calculating confidence score with scoring system',
    );
    const score = scoringSystem.calculateConfidenceScore(
      result,
      context,
      aiResult,
      heuristicResult,
  ...
  public recordAgentPerformance(
    ...
    scoringSystem.recordPerformanceFeedback(
      agentType,
      success,
      responseTime,
      userSatisfaction,
  ...
  private extractParametersFromAI(
    aiParameters: any,
    agentType: AgentType,
    context?: IntentAnalysisContext,
  ): Record<string, any> {
    ...
    switch (type) {
      ...
      case 'error':
        this.metrics.errorRate =
          (this.metrics.errorRate * (this.metrics.totalAnalyses - 1) + 1) /
          this.metrics.totalAnalyses;
        break;
    }
  ...
  private initializeLanguageKeywords(): Record<
    string,
  ...
  public detectAgentExclusions(query: string): AgentExclusionResult {
    console.log(
      '[IntentAnalysis] Detecting agent exclusions for query:',
      query.substring(0, 100),
    );
    ...
    const {
      AGENT_EXCLUSION_PATTERNS,
    } = require('./intentClassificationPrompts');

    // Check each agent type for exclusion patterns
    for (const [agentType, patterns] of Object.entries(
      AGENT_EXCLUSION_PATTERNS,
    )) {
      ...
      for (const pattern of typedPatterns.avoidancePatterns) {
        if (queryLower.includes(pattern.toLowerCase())) {
          ...
          console.log(
            `[IntentAnalysis] Found avoidance pattern for ${agentType}:`,
            pattern,
          );
        }
      }
      ...
      for (const pattern of typedPatterns.negativePatterns) {
        if (pattern.test(query)) {
          ...
          console.log(
            `[IntentAnalysis] Found negative pattern for ${agentType}:`,
            pattern.toString(),
          );
        }
      }
      ...
      for (const keyword of typedPatterns.exclusionKeywords) {
        if (queryLower.includes(keyword.toLowerCase())) {
          ...
          console.log(
            `[IntentAnalysis] Found exclusion keyword for ${agentType}:`,
            keyword,
          );
        }
      }
  ...
  private applyUserExclusions(
    ...
    if (exclusions.excludedAgents.includes(agentType)) {
      const penalizedConfidence = Math.max(
        0.05,
        currentConfidence - exclusions.confidencePenalty,
      );
      console.log(
        `[IntentAnalysis] Applied exclusion penalty to ${agentType}:`,
        {
 ...
 */
export function getIntentAnalysisService(
  config?: Partial<IntentAnalysisConfig>,
): IntentAnalysisService {
...
export function recordIntentAnalysisPerformance(
  ...
  const service = getIntentAnalysisService();
  service.recordAgentPerformance(
    agentType,
    success,
    responseTime,
    userSatisfaction,
