...
export class BingGroundingService {
  ...
  constructor(config?: Partial<BingGroundingConfiguration>) {
    ...
    this.bingGroundingAgentId =
      config?.bingGroundingAgentId ||
      process.env.AZURE_BING_GROUNDING_AGENT_ID ||
      process.env.AZURE_AI_AGENT_ID ||
      '';
    this.bingGroundingAgentName =
      config?.bingGroundingAgentName ||
    ...
    try {
      ...
      if (!this.logger) {
        ...
        console.warn(
          'AzureMonitorLoggingService: Missing required environment variables for Azure Monitor Logging',
        );
        console.warn(
          'Set LOGS_INGESTION_ENDPOINT, DATA_COLLECTION_RULE_ID, and STREAM_NAME to enable logging',
        );
    ...
    if (configErrors.length > 0) {
      console.warn(
        '[WARN] Azure AI Agents configuration incomplete:',
        configErrors.join(', '),
      );
      ...
      if (!this.connectionId) {
        console.warn(
          '[SETUP] To enable Azure AI Agents, set AZURE_GROUNDING_CONNECTION_ID environment variable',
        );
      }
      if (!this.projectEndpoint) {
        console.warn(
          '[SETUP] To enable Azure AI Agents, set PROJECT_ENDPOINT environment variable',
        );
        this.projectEndpoint =
    ...
    console.log(
      '[INFO] BingGroundingService initialized with connection:',
      this.connectionId.substring(0, 20) + '...',
    );
  }
  ...
  private async initializeAzureAgents(): Promise<void> {
    try {
      ...
      if (
        ...
        console.warn(
          '[WARN] Azure AI Agents initialization skipped - Invalid PROJECT_ENDPOINT:',
          this.projectEndpoint,
        );
        console.warn(
          '[SETUP] Set PROJECT_ENDPOINT to your Azure AI Foundry project endpoint (e.g., https://my-project.cognitiveservices.azure.com/)',
        );
  ...
  async search(request: WebSearchRequest): Promise<WebSearchResponse> {
    ...
    try {
      ...
      if (cachedResponse) {
        console.log(
          '[INFO] Returning cached search results for query:',
          request.query.substring(0, 50),
        );
        await this.logger?.logCustomMetric(
          'BingGroundingCacheHit',
          1,
          'count',
          {
      ...
      const response = this.processSearchResults(
        searchResults,
        request,
        startTime,
      );
  ...
  private async executeSearch(
    ...
    try {
      ...
      if (this.agentsClient && this.initializationComplete) {
        try {
          console.log(
            '[INFO] Attempting Azure AI Agents search for query:',
            request.query.substring(0, 50),
          );

          // Get existing agent (from environment variable)
          const agentId =
            this.bingGroundingAgentId || process.env.AZURE_AI_AGENT_ID;
          if (!agentId) {
            throw new Error(
              'Agent ID is required. Set AZURE_BING_GROUNDING_AGENT_ID or AZURE_AI_AGENT_ID',
            );
          }
          ...
          if (run.status !== 'completed') {
            throw new Error(
              `Azure AI run timed out with status: ${run.status}`,
            );
          }
          ...
          console.warn(
            '[WARN] Azure AI Agents execution failed, falling back to placeholder:',
            agentError,
          );
          ...
          if (agentError && typeof agentError === 'object') {
            ...
            if (
              ...
              console.error(
                '[ERROR] Azure AI Agents authentication failed. Check DefaultAzureCredential setup.',
              );
            }
            ...
            if (
              ...
              console.error(
                '[ERROR] Azure AI Agents rate limit exceeded. Consider implementing backoff strategy.',
              );
            }
            ...
            if (error.statusCode === 503 || error.message?.includes('quota')) {
              console.error(
                '[ERROR] Azure AI Agents service unavailable or quota exceeded.',
              );
            }
          }
        ...
        console.log(
          '[DEV] Azure AI Agents unavailable - Reasons:',
          reasons.join(', '),
          '| Query:',
          request.query.substring(0, 50),
        );
          ...
          search_results: [
            {
              ...
              snippet:
                'Azure AI Agents configuration is required for web search functionality. Please set up PROJECT_ENDPOINT and AZURE_AI_AGENT_ID environment variables.',
  ...
  private processSearchResults(
    ...
    try {
      ...
      const { assistantContent, assistantAnnotations } =
        this.extractAssistantContent(azureResponse);
      ...
      return {
        ...
        citations,
        totalCount:
          azureResponse.metadata?.total_estimated_matches || results.length,
        searchTime,
        ...
        metadata: {
          ...
          azureMetadata: {
            bingSearchUrl,
            optimizedQuery:
              azureResponse.metadata?.optimized_query || request.query,
   ...
   */
  private extractWebSearchResults(
    azureResponse: AzureAgentsResponse,
  ): WebSearchResult[] {
    try {
        ...
        azureResponse.runSteps.forEach((step) => {
          if (step.step_details?.tool_calls) {
            step.step_details.tool_calls.forEach((toolCall, callIndex) => {
              if (
                toolCall.type === 'bing_grounding' &&
                toolCall.bing_grounding
              ) {
                ...
                if (bingData.results && Array.isArray(bingData.results)) {
                  bingData.results.forEach((result: any, index: number) => {
                    results.push({
                      ...
                      displayUrl:
                        result.displayUrl || this.extractDisplayUrl(result.url),
                      dateLastCrawled:
                        result.dateLastCrawled || result.datePublished,
                ...
                // Based on C# docs line 319-322: toolCall.BingGrounding contains key-value pairs
                  ...
                  console.log(
                    '[INFO] Parsing Bing Grounding key-value pairs:',
                    Object.keys(bingData),
                  );
                  ...
                  if (bingData.status === 'placeholder_mode') {
                    ...
                    console.log(
                      '[INFO] Skipping placeholder mode tool call data',
                    );
                    ...
                    results.push({
                      ...
                      displayUrl:
                        bingData.displayUrl ||
                        this.extractDisplayUrl(bingData.url || ''),
                      language: 'en',
                      contentType: 'webpage',
                      relevanceScore: 0.8,
                      metadata: {
      ...
      if (results.length === 0 && azureResponse.metadata?.search_results) {
        azureResponse.metadata.search_results.forEach(
                ...
                content.text.annotations.forEach((annotation, index) => {
                  ...
                  if (
                    ...
                    results.push({
                      ...
                      displayUrl: this.extractDisplayUrl(
                    ...
                    annotation.type === 'uri_citation' &&
                    annotation.uri_citation
                  ) {
                    results.push({
                      id: `citation_result_${msgIndex}_${index}`,
                      title:
                        annotation.uri_citation.title ||
                        annotation.text ||
                        'Untitled',
                      url: annotation.uri_citation.uri,
                      snippet: annotation.text || 'No snippet available',
                      displayUrl: this.extractDisplayUrl(
                        annotation.uri_citation.uri,
                      ),
                      language: 'en',
                      contentType: 'webpage',
                      relevanceScore: Math.max(0.1, 1 - index * 0.1),
      ...
      console.log(
        `[INFO] Extracted ${results.length} search results from Azure AI Agents response`,
      );
      return results;
    } catch (error) {
      console.error(
        '[ERROR] Failed to extract search results from Azure response:',
        error,
      );
      return [];
    }
   ...
   */
  private extractCitationsFromAnnotations(
    azureResponse: AzureAgentsResponse,
  ): Citation[] {
    ...
    try {
              ...
              content.text.annotations.forEach((annotation) => {
                ...
                if (
                  ...
                  citations.push({
                    ...
                    type: this.determineCitationTypeFromUrl(
                      annotation.urlCitation.url,
                    ),
                    metadata: {
                      ...
                      startIndex:
                        annotation.startIndex || annotation.start_index,
      ...
      console.error(
        '[ERROR] Failed to extract citations from annotations:',
        error,
      );
      return [];
    }
  ...
  private extractAssistantContent(azureResponse: AzureAgentsResponse): {
    ...
    try {
      ...
      const assistantMessages = azureResponse.messages.filter(
        (m) => m.role === 'assistant',
      );
          ...
          if (content.text.annotations) {
            content.text.annotations.forEach((annotation) => {
              ...
              const mappedAnnotation: any = {
                ...
                startIndex:
                  annotation.startIndex || annotation.start_index || 0,
      ...
      return {
        ...
        assistantAnnotations:
          assistantAnnotations.length > 0 ? assistantAnnotations : undefined,
      };
    } catch (error) {
  ...
  private generateBingSearchUrl(request: WebSearchRequest): string {
    ...
    if (request.freshness) {
      ...
      params.set(
        'qft',
        `filterui:age-lt${freshnessMap[request.freshness] || 'week'}`,
      );
    }
  ...
  private parseSearchResultsFromContent(content: string): any[] {
    ...
    try {
      ...
      urls.forEach((url, index) => {
        ...
        const afterUrl = content.substring(
          urlIndex + url.length,
      ...
      console.error(
        '[ERROR] Failed to parse search results from content:',
        error,
      );
      return [];
    }
  ...
  private determineCitationTypeFromUrl(url: string): Citation['type'] {
    ...
    if (
      ...
      urlLower.match(/\.(com|org|net)\/\d{4}\/\d{2}/)
    ) {
      return 'news';
    }
    if (
  ...
  private validateConfiguration(): string[] {
    ...
    if (this.connectionId && !this.connectionId.startsWith('/subscriptions/')) {
      errors.push(
        'AZURE_GROUNDING_CONNECTION_ID should be a full Azure resource path (e.g., /subscriptions/.../connections/...)',
      );
    }
    ...
    const hasExistingAgent = !!(
      this.bingGroundingAgentId || this.bingGroundingAgentName
    );
    if (!hasExistingAgent) {
      // If no existing agent specified, warn that agent creation requires additional permissions
      console.log(
        '[INFO] No existing agent specified. Will attempt to create agent dynamically (requires foundry write permissions)',
      );
      console.log(
        '[SETUP] To use existing agent, set AZURE_BING_GROUNDING_AGENT_ID or AZURE_BING_GROUNDING_AGENT_NAME',
      );
    } else {
      console.log(
        '[INFO] Using existing agent:',
        this.bingGroundingAgentId || this.bingGroundingAgentName,
      );
    }
   ...
   */
  public async discoverBingGroundingAgents(): Promise<
    Array<{
      id: string;
      name: string;
  ...
  > {
    try {
      ...
      console.log(
        '[INFO] Discovering available agents with Bing Grounding tools...',
      );
      ...
      console.log(
        `[INFO] Found ${agents.length} agents, ${
          agents.filter((a) => a.hasBingGrounding).length
        } with Bing Grounding`,
      );
      return agents;
    } catch (error) {
    ...
    try {
      ...
      if (!this.connectionId || !this.projectEndpoint) {
        return {
          success: false,
          error:
            'Missing required configuration (connectionId or projectEndpoint)',
        };
      }
   ...
   */
  public static createConfigForUseCase(
    useCase: 'news' | 'academic' | 'general' | 'recent',
  ): Partial<BingGroundingConfiguration> {
