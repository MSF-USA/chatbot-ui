...
export class ConfidenceScoringSystem {
   ...
   */
  public static getInstance(weights?: Partial<ConfidenceWeights>): ConfidenceScoringSystem {
  ...
  public calculateConfidenceScore(
    ...
    try {
      const factors = this.extractConfidenceFactors(result, context, aiResult, heuristicResult);
      const weightedScore = this.calculateWeightedScore(factors);
      const adjustedScore = this.applyConfidenceAdjustments(weightedScore, result, context);
  ...
  public recordPerformanceFeedback(
    ...
    try {
      ...
      historical.successRate = alpha * (success ? 1 : 0) + (1 - alpha) * historical.successRate;
      historical.totalExecutions++;
      historical.averageResponseTime = alpha * responseTime + (1 - alpha) * historical.averageResponseTime;
      
      if (userSatisfaction !== undefined) {
        historical.userSatisfaction = alpha * userSatisfaction + (1 - alpha) * historical.userSatisfaction;
      }
  ...
  public explainConfidence(
    ...
    const explanation = `Confidence level: ${level} (${(result.confidence * 100).toFixed(1)}%). ` +
  ...
  public calibrateConfidence(
    ...
    try {
      ...
      const calibrationFactor = actualSuccessRate / Math.max(0.1, expectedSuccessRate);
      ...
      const finalConfidence = rawConfidence * (1 - smoothingFactor) + 
  ...
  private extractConfidenceFactors(
    ...
    return {
      ...
      keywordRelevance: this.calculateKeywordRelevance(context.query, result.recommendedAgent),
      historicalSuccess: this.calculateHistoricalFactor(result.recommendedAgent),
      contextSimilarity: this.calculateContextSimilarity(context),
      userPreference: this.calculateUserPreference(context, result.recommendedAgent),
  ...
  private calculateAIConfidenceFactor(aiResult?: IntentClassificationResponse): number {
  ...
  private calculateHeuristicFactor(heuristicResult?: HeuristicAnalysisResult): number {
    ...
    const patternBonus = Math.min(0.2, heuristicResult.matchedPatterns.length * 0.05);
    
    // Different pattern types have different weights
    const hasUrl = heuristicResult.matchedPatterns.some(p => p.match(/https?:\/\//));
    const hasCode = heuristicResult.matchedPatterns.some(p => p.match(/```|function|def/));
    const hasTime = heuristicResult.matchedPatterns.some(p => ['today', 'recent', 'latest'].includes(p.toLowerCase()));
  ...
  private calculateKeywordRelevance(query: string, agentType: AgentType): number {
    const queryLower = query.toLowerCase();
    const agentKeywords = this.getAgentKeywords(agentType);
    
    const matchCount = agentKeywords.filter(keyword => queryLower.includes(keyword)).length;
    const relevanceScore = Math.min(1, matchCount / Math.max(1, agentKeywords.length * 0.3));
  ...
  private calculateContextSimilarity(context: IntentAnalysisContext): number {
    ...
    const intersection = new Set([...currentWords].filter(word => historyWords.has(word)));
    const similarity = intersection.size / Math.max(1, currentWords.size);
    
    return Math.min(1, similarity * 2); // Scale up similarity score
  }

  private calculateUserPreference(context: IntentAnalysisContext, agentType: AgentType): number {
  ...
  private calculateSignalStrength(result: IntentAnalysisResult, context: IntentAnalysisContext): number {
    let strength = 0;
  ...
  private calculateTemporalRelevance(query: string): number {
    const timeWords = ['now', 'today', 'current', 'latest', 'recent', 'breaking', 'live'];
    const hasTimeContext = timeWords.some(word => query.toLowerCase().includes(word));
  ...
  private calculateParameterQuality(parameters: Record<string, any>): number {
    ...
    if (parameters.urls && Array.isArray(parameters.urls) && parameters.urls.length > 0) {
      quality += 0.2;
    }
    if (parameters.query && typeof parameters.query === 'string' && parameters.query.length > 5) {
      quality += 0.1;
    }
  ...
  private applyConfidenceAdjustments(
    ...
    if (context.query.match(/https?:\/\//) && result.recommendedAgent === AgentType.URL_PULL) {
      adjusted += 0.15;
    }
    if (context.query.match(/```/) && result.recommendedAgent === AgentType.CODE_INTERPRETER) {
      adjusted += 0.12;
    }
  ...
  private getAgentKeywords(agentType: AgentType): string[] {
    const keywords: Record<AgentType, string[]> = {
      [AgentType.WEB_SEARCH]: ['search', 'find', 'latest', 'recent', 'current', 'news', 'information'],
      [AgentType.CODE_INTERPRETER]: ['code', 'program', 'function', 'debug', 'python', 'javascript', 'calculate'],
      [AgentType.URL_PULL]: ['website', 'url', 'analyze', 'page', 'site', 'link'],
      [AgentType.LOCAL_KNOWLEDGE]: ['company', 'internal', 'policy', 'procedure', 'documentation'],
      [AgentType.THIRD_PARTY]: ['github', 'slack', 'api', 'integration', 'service'],
 ...
 */
export function getConfidenceScoringSystem(weights?: Partial<ConfidenceWeights>): ConfidenceScoringSystem {
...
export function calculateIntentConfidence(
  ...
  return scoring.calculateConfidenceScore(result, context, aiResult, heuristicResult);
}